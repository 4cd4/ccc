# Run VaultMind in production without Docker
...
Here you can find the scripts and known working process to run VaultMind outside of a Docker container.
...
`git clone git@github.com:4cd4/ccc.git`

2. `cd ccc` and run `yarn setup`. This will install all dependencies to run in production as well as debug the application.
...
VaultMind is comprised of three main sections. The `frontend`, `server`, and `collector`. When running in production you will be running `server` and `collector` on two different processes, with a build step for compilation of the frontend.
...
VaultMind should now be running on `http://localhost:3001`!

## Updating VaultMind

To update VaultMind with future updates you can `git pull origin master` to pull in the latest code and then repeat steps 2 - 5 to deploy with all changes fully.
...
_note_ You should `pkill node` before running an update so that you are not running multiple VaultMind processes on the same instance as this can cause conflicts.
...
cd $HOME/ccc/frontend && yarn && yarn build && cd $HOME/ccc
...
cd $HOME/ccc/collector && yarn

echo "Installing server dependencies & running migrations"
cd $HOME/ccc/server && yarn
cd $HOME/ccc/server && npx prisma migrate deploy --schema=./prisma/schema.prisma
cd $HOME/ccc/server && npx prisma generate
