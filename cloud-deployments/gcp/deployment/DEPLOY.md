...
2. After successful login, Run the following command to create a deployment using the Deployment Manager CLI:
  ...
  gcloud deployment-manager deployments create vaultmind-deployment --config gcp/deployment/gcp_deploy_anything_llm.yaml
...
```
gcloud compute instances get-serial-port-output vaultmind-instance 
```

ssh into the instance

```
gcloud compute ssh vaultmind-instance 
```

Delete the deployment
```
gcloud deployment-manager deployments delete vaultmind-deployment 
```
