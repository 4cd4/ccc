...
// DISABLE_TELEMETRY=true in the `.env` of however you setup.
...
async function setupTelemetry() {
  if ((process.env.DISABLE_TELEMETRY ?? "true") === "true") {
    console.log(
      `\x1b[31m[TELEMETRY DISABLED]\x1b[0m Telemetry is marked as disabled - no events will send.`
    );
    return true;
  }
  ...
  console.log(
    `\x1b[32m[TELEMETRY ENABLED]\x1b[0m Anonymous Telemetry enabled. Telemetry helps improve VaultMind.`
  );
